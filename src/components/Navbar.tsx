...
export default function Navbar() {
  ...
  return (
    <div className="w-full">
      <div className={`${navigation ? "absolute bg-black opacity-50 lg:opacity-0 bottom-0 top-0 z-10 left-0 right-0" : ""}`}></div>
      <nav className={`bg-white px-10 lg:px-20 py-5 flex shadow items-center justify-between`}>
        <img src={Logo} alt="logo-image" width={180}/>
        {
          navigation ? (
            <>
              <Icon icon={"ph:x-bold"} width={30} className="flex lg:hidden cursor-pointer" onClick={handleNavigation}/>
            </>
            ) : (
            <>
              <Icon icon={"fluent:navigation-16-filled"} width={30} className="flex lg:hidden cursor-pointer" onClick={handleNavigation}/>
            </>
          )
        }
        {
          navigation ? (
            <>
              <div className="flex flex-col px-10 py-7 w-3/5 md:w-2/5 text-sm font-medium gap-7 absolute z-20 bg-white right-0 top-0 bottom-0">
                <Icon icon={"ph:x-bold"} width={30} className="flex cursor-pointer" onClick={handleNavigation}/>
                <Link 
                  to={"/"} 
                ...
                <Link 
                  to={"/penerbangan"} 
                  className={handleActivePage("/penerbangan")} onClick={handleNavigation}
                >
                  Penerbangan
                </Link>
                <Link 
                ...
                <div 
                  ...
                  className={`flex items-center gap-2 cursor-pointer relative ${handleActivePage("/profile")}`}>
                  <details className="w-full">
                    ...
                    <div className="border mt-3 rounded">
                      <Link to={"/profile"} onClick={handleNavigation}>
                        <h6 className={`p-3 border-b hover:bg-[#F5F5F5] ${location.pathname === "/profile" ? "bg-[#F5F5F5]" : ""}`}>{fullName}</h6>
                      </Link>
                      <h6 className="p-3 text-black hover:bg-[#F5F5F5]" onClick={handleLogout}>Logout</h6>
                    </div>
                  </details>
                </div>
            </div>
